/** GET /api/scrape-leads/progress/[jobId] — proxy za progress events (SSE). */
import type { APIRoute } from "astro";
export const GET: APIRoute = async ({ params }) => {
  const url = process.env.SCRAPER_LEADS_URL ?? "http://127.0.0.1:8002";
  try {
    const r = await fetch(`${url}/scrape-leads/progress/${params.jobId}`, { signal: AbortSignal.timeout(30_000) });
