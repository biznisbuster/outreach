...
export function buildWhere(f: LeadFilterValues): SQL | undefined {
  ...
  if (f.lastEmailStatus !== "any") {
    conds.push(
      sql`(SELECT status FROM email_sends WHERE lead_id = ${schema.leads.id} ORDER BY COALESCE(sent_at, queued_at, created_at) DESC LIMIT 1) = ${f.lastEmailStatus}`,
    );
  }
...
const SORT_COLS: Record<string, SQL | typeof schema.leads[keyof typeof schema.leads]> = {
  ...
  lastEmail: sql`(SELECT COALESCE(sent_at, queued_at, created_at) FROM email_sends WHERE lead_id = ${schema.leads.id} ORDER BY COALESCE(sent_at, queued_at, created_at) DESC LIMIT 1)`,
};
